...
class Application_Model_RegistryDocumentsTemplates extends Muzyka_DataModel
{
    ...
    public function getRegistryDocumntTemplate($registry_id)
    {
       $result = $this->_db->query(sprintf('SELECT * FROM `registry_documents_templates` where `registry_id` = "'.$registry_id.'"'))->fetchAll(PDO::FETCH_ASSOC);
       return $result;
    }
}
